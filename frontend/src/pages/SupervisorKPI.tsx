...
import { kpiCity } from '../shared/api/kpi';
...
export default function SupervisorKPI(){
  ...
  useEffect(()=>{ fetchMeProfile().then(me=>{ setCity(me?.city_code||''); }); }, []);
  useEffect(()=>{ if(!city) return; kpiCity(city).then(setData); }, [city]);

  if (!data) return <div className="p-4">Загрузка…</div>;
  return (
    <div className="p-4 space-y-3">
      <h2 className="text-xl font-semibold">KPI города {data.city} ({data.month})</h2>
      ...
      <div className="p-3 border rounded overflow-auto">
        <table className="min-w-[800px] text-sm">
          <thead><tr className="bg-gray-50"><th className="p-2">Store</th><th className="p-2">Название</th><th className="p-2">План</th><th className="p-2">Факт</th><th className="p-2">%</th></tr></thead>
          <tbody>
            {(data.stores||[]).map((r:any, idx:number)=>(
              <tr key={idx} className="border-t">
                <td className="p-1">{r.store_id}</td>
                <td className="p-1">{r.store_name}</td>
