...
depends_on = None
def upgrade() -> None:
    op.create_table('store_coefficients',
    ...
    op.create_unique_constraint('uq_store_coeff_key', 'store_coefficients', ['store_id','code','valid_from'])
    op.create_index('ix_store_coeff_active', 'store_coefficients', ['store_id','code','valid_from','valid_to'], unique=False)
def downgrade() -> None:
