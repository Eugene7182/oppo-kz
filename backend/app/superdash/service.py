...
def _stores_for_super(db: Session, supervisor: str | None):
    ...
    if supervisor:
        return _df(
            db,
            f"select store_id, city_code, store_name, network_id from {TBL_STORES} where supervisor_name=:s",
            {"s": supervisor},
        )
...
def promoter_sales(
    ...
    if supervisor and _has(db, TBL_STORES):
        store_ids = _df(
            db, "select store_id from stores where supervisor_name=:s", {"s": supervisor}
        )
        if not store_ids.empty:
            cond += " and store_id = any(:stores)"
            params["stores"] = store_ids["store_id"].tolist()
...
def summary(
    db: Session, supervisor: str | None = None, scope: str = "week", city: str | None = None
):
    if not _has(db, TBL_SALES):
    ...
    if supervisor and _has(db, TBL_STORES):
        store_ids = _df(
            db, "select store_id, city_code from stores where supervisor_name=:s", {"s": supervisor}
        )
        if not store_ids.empty:
            if city:
    ...
    return {
        "period": {"scope": scope},
        "total": ({"qty": 0, "revenue": 0} if df.empty else df.iloc[0].to_dict()),
    }
...
def weekly_vs(db: Session, supervisor: str | None = None, city: str | None = None, weeks: int = 8):
    ...
    df = _df(
        db,
        f"select to_char(date_trunc('week', date), 'IYYY-IW') as week, sum(qty) qty from {TBL_SALES} {cond} group by 1 order by 1 desc limit :n",
        {**params, "n": int(weeks)},
    )
