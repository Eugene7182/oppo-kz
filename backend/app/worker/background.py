...
import threading, io, csv
from typing import Callable
from app.db.session import SessionLocal
from app.services.jobs import mark_running, update_progress, mark_done, mark_error
...
def run_sales_import_job(db, *, job_id: int, source: str, file_bytes: bytes):
    try:
        ...
        if total == 0:
            update_progress(db, job_id, processed=0, total=0); mark_done(db, job_id); return
        processed = 0
        def progress_cb(done: int):
            nonlocal processed
            processed = done
            update_progress(db, job_id, processed=processed, total=total)
        res = import_sales_csv(db, file_bytes, source=source, dry_run=False, progress_cb=progress_cb)
        if not res.get("ok"):
