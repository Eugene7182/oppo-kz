...
from ....core.security import get_db, require_roles, get_current_user
...
@router.get("/", response_model=List[StoreCoefficientOut])
...
@router.post("/", response_model=StoreCoefficientOut, dependencies=[Depends(require_roles("super"))])
...
@router.put("/{coeff_id}", response_model=StoreCoefficientOut, dependencies=[Depends(require_roles("super"))])
def update_coeff(coeff_id: int, body: StoreCoefficientIn, db: Session = Depends(get_db)):
    ...
    db.add(coeff); db.commit(); db.refresh(coeff); return coeff
@router.delete("/{coeff_id}", status_code=204, dependencies=[Depends(require_roles("super"))])
