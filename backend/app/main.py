...
def _ensure_user_table_schema() -> None:
    """
    Доворачиваем таблицу user до ожидаемой схемы:
      - email UNIQUE
    ...
    with engine.begin() as conn:
        # full_name
        if not _column_exists("public", "user", "full_name"):
            print("WARNING:app.main:user.full_name is missing — adding")
            conn.execute(text('ALTER TABLE public."user" ADD COLUMN full_name VARCHAR(255)'))
        # password_hash
        if _column_exists("public", "user", "hashed_password") and not _column_exists(
            "public", "user", "password_hash"
        ):
            print("WARNING:app.main:user.hashed_password found — renaming to password_hash")
            conn.execute(
                text('ALTER TABLE public."user" RENAME COLUMN hashed_password TO password_hash')
            )
        if not _column_exists("public", "user", "password_hash"):
            conn.execute(text('ALTER TABLE public."user" ADD COLUMN password_hash VARCHAR(255)'))
        # is_active
        if not _column_exists("public", "user", "is_active"):
            print("WARNING:app.main:user.is_active is missing — adding with default true")
            conn.execute(
                text('ALTER TABLE public."user" ADD COLUMN is_active BOOLEAN DEFAULT TRUE')
            )
        # email UNIQUE + индекс
    ...
    print("INFO:app.main:user table schema ensured")


def _ensure_user_role_varchar() -> None:
    """
    Приводим user.role из enum userrole в VARCHAR(20),
    ...
    with engine.begin() as conn:
        ...
        if str(data_type).upper() == "USER-DEFINED":
            ...
            try:
                conn.execute(text('ALTER TABLE public."user" ALTER COLUMN role DROP DEFAULT'))
            except Exception:
            ...
            conn.execute(text("ALTER TABLE public.\"user\" ALTER COLUMN role SET DEFAULT 'admin'"))
            ...
            print("INFO:app.main:user.role converted to VARCHAR(20)")


def _ensure_user_id_varchar36() -> None:
    """
    Безопасно приводим user.id к VARCHAR(36), если он ещё integer.
    Если есть FK из invites.invited_by → user.id, временно снимаем fk.
    """
    if engine.dialect.name != "postgresql":
        return
    with engine.begin() as conn:
        ...
        if str(data_type).lower() in ("integer", "bigint", "smallint"):
            print("INFO:app.main:user.id converted to VARCHAR(36)")
            ...
            try:
                ...
                invited_by_exists = conn.execute(
                    text(
                        """
                    SELECT 1
                    FROM information_schema.columns
...
def seed_admin(db: Session) -> None:
    ...
    role записывается строкой 'admin' (после _ensure_user_role_varchar()).
    """
    ...
    col_type = _get_column_type("public", "user", "role")
    if col_type and str(col_type).upper() == "USER-DEFINED":
        with engine.begin() as conn:
            ...
            conn.execute(
                text(
                    """
                    INSERT INTO public."user" (id, email, full_name, role, password_hash, is_active)
...
def on_startup():
    ...
    create_all()
    # 2) доворачиваем user: колонки/индексы
    _ensure_user_table_schema()
    # 3) приводим user.role (enum → VARCHAR(20))
    _ensure_user_role_varchar()
    # 4) приводим id к VARCHAR(36) — на случай старой БД; если уже в порядке — ничего не делает
    _ensure_user_id_varchar36()
    # 5) сидируем админа
