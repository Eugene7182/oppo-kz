...
smoke:
	...
	bash -lc "BASE_URL=$$BASE_URL ADMIN_USER=$$ADMIN_USER ADMIN_PASS=$$ADMIN_PASS ./scripts/smoke.sh"


wipe:
	CONFIRM?=NO; DRY_RUN?=true; \
	bash -lc "CONFIRM=$$CONFIRM DRY_RUN=$$DRY_RUN ./scripts/wipe_demo.sh"

seed-full:
	bash -lc "./scripts/seed_full.sh"

load-sales:
	DAYS?=30; PER_DAY?=50; STORES?=A01,A02; SKUS?=OPPO-A1K,OPPO-RENO10,OPPO-ENCOBUDS2; PROMOTER?=pavlov; \
	bash -lc "DAYS=$$DAYS PER_DAY=$$PER_DAY STORES=$$STORES SKUS=$$SKUS PROMOTER=$$PROMOTER ./scripts/seed_sales_bulk.sh"
